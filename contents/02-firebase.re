...
//abstract{
  ...
  ユーザがチャットではなくゲームデータをリアルタイムに交換しているのをみて、
...
//blankline
ネットの世界では、スタートアップはツブされるか買収されるかの2択なので、結局Googleに買収されてしまう。

//blankline
Googleは、更にサービスを追加しモバイル・バックエンド・サービス、サーバーレス・アーキテクチャの勝者となります。
//}
...
//blankline
Googleアカウントがあれば、スグに使い始めることが出来ますし、ある程度の使用量は無償です。

//image[firebase00][][scale=0.7 ,pos=H]

料金については、

Firebase Authentication（認証）では、以下のようにアクティブユーザが月間5万人まで無料です。
...
最初にプロジェクトに名前を付けます。使える文字は半角英数、スペース、-!'"の記号4つです。[続行]ボタンをクリックします。
