...
//blankline
「State-Management」とは、デザインパターンによる実装であり、
...
//blankline
上記の例ですと、ボタンの形を変えるためにAモジュールにアクセスし、色を変えるためにBモジュールにアクセスするような実装ではなく
ボタンの状態を管理するモジュールがあれば、そこにアクセスするだけで良いとする実装を指します。
...
コントローラーは、「GetXコントローラー」を継承し、データとビジネスロジックを持っています。
...
では、状態管理コントローラーを導入したメリットは何でしょうか？
 ...
 1. データとビジネスロジックを一カ所にまとめたので、変更があっても一カ所。
...
表示部を2つ、それぞれを操作するボタンを2つにしましたが、状態管理コントローラーをひとつ増やしただけです。
こちらには、優秀な「GetX<コントローラー型>」を使って表示しています。
//image[ch01_05_state_management][][scale=0.7 ,pos=H]

//blankline
==== 状態管理コントローラーの別インスタンス

同じデータ型、同じビジネスロジックを持つのであれば、状態管理コントローラーの別インスタンスで管理できます。
...
ページ移動を行っても、アプリケーション全体で状態管理コントローラーは動作していますので、
状態管理コントローラーへアクセスすれば、先ほどまでのデータを取得できます。
