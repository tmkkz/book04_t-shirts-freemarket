...
準備さえできれば簡単に実装できます。ユーザーのメリットは、アプリケーション提供側へパスワードの登録がなく、
...
//image[04_twitterDev02][][scale=0.4, pos=H, border=on]

//clearpage
続いて、「Twitter API」を選択します。
//image[04_twitterDev03][][scale=0.4, pos=H, border=on]
...
//blankline
コールバックURLは、「適当な英数字://」でかまいません。これは、Firebaseコンソールで指定するURLが
のちほどインストールするプラグインで使えないためです。
...
//blankline
Twitterアカウントでのサインインを実装するときには、
Twitter Developperサイトで取得した、
...
//blankline
しかし、コードに直接上記のキーを書込みすると危険です。ソースコードをGitHubなどで管理している場合「公開」になっていますと
間違いなく悪用されますし、GitHubの場合には警告メールがきます。
...
//blankline
その際、下から3行目の「example」をTwitterへ登録したコールバックURLにします。「://」は不要です。
...
認証コントローラ（auth_controller.dart）へメソッドを追加します。
