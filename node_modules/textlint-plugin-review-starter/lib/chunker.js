...
exports.ChunkTypes = void 0;
exports.parseAsChunks = parseAsChunks;
var ChunkTypes = {
  Paragraph: 'Paragraph',
  Heading: 'Heading',
...
function parseAsChunks(text) {
  ...
  var isComment = false;
  var chunks = lines.reduce(function (result, currentLine, index) {
    var line = {
      raw: currentLine,
      // with line endings
  ...
  chunks.forEach(function (chunk) {
    var firstLineIndex = chunk.lines[0].lineNumber - 1;
    var lastLineIndex = chunk.lines[chunk.lines.length - 1].lineNumber - 1;
  ...
  function parseLine(result, line) {
    ...
    if (line.text.match(/^\/\/\w+/)) {
      flushChunk();
      var chunk = createChunk(ChunkTypes.Block, line);
      result.push(chunk);
    ...
    if (line.text.match(/^\s+/) && currentChunk && currentChunk.type === ChunkTypes.DefinitionList) {
      currentChunk.lines.push(line);
      return;
    } // empty line
